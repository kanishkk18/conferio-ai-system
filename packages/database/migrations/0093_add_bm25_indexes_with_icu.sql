...
-- );
-- Custom SQL migration file, put your code below! --
-- Replaced BM25 (ParadeDB) indexes with PostgreSQL native GIN indexes for Neon compatibility

-- 1. agents
DROP INDEX IF EXISTS agents_fts_idx;
CREATE INDEX agents_fts_idx ON agents USING GIN (to_tsvector('english', coalesce(title, '') || ' ' || coalesce(description, '') || ' ' || coalesce(system_role, '')));

-- 2. topics
DROP INDEX IF EXISTS topics_fts_idx;
CREATE INDEX topics_fts_idx ON topics USING GIN (to_tsvector('english', coalesce(title, '') || ' ' || coalesce(content, '') || ' ' || coalesce(description, '')));

-- 3. files
DROP INDEX IF EXISTS files_fts_idx;
CREATE INDEX files_fts_idx ON files USING GIN (to_tsvector('english', coalesce(name, '')));

-- 4. knowledge_bases
DROP INDEX IF EXISTS knowledge_bases_fts_idx;
CREATE INDEX knowledge_bases_fts_idx ON knowledge_bases USING GIN (to_tsvector('english', coalesce(name, '') || ' ' || coalesce(description, '')));

-- 5. user_memories
DROP INDEX IF EXISTS user_memories_fts_idx;
CREATE INDEX user_memories_fts_idx ON user_memories USING GIN (to_tsvector('english', coalesce(title, '') || ' ' || coalesce(summary, '') || ' ' || coalesce(details, '')));

-- 6. chat_groups
DROP INDEX IF EXISTS chat_groups_fts_idx;
CREATE INDEX chat_groups_fts_idx ON chat_groups USING GIN (to_tsvector('english', coalesce(title, '') || ' ' || coalesce(description, '') || ' ' || coalesce(content, '')));

-- 7. user_memories_contexts
DROP INDEX IF EXISTS user_memories_contexts_fts_idx;
CREATE INDEX user_memories_contexts_fts_idx ON user_memories_contexts USING GIN (to_tsvector('english', coalesce(title, '') || ' ' || coalesce(description, '')));

-- 8. user_memories_preferences
DROP INDEX IF EXISTS user_memories_preferences_fts_idx;
CREATE INDEX user_memories_preferences_fts_idx ON user_memories_preferences USING GIN (to_tsvector('english', coalesce(conclusion_directives, '') || ' ' || coalesce(suggestions, '')));

-- 9. user_memories_activities
DROP INDEX IF EXISTS user_memories_activities_fts_idx;
CREATE INDEX user_memories_activities_fts_idx ON user_memories_activities USING GIN (to_tsvector('english', coalesce(notes, '') || ' ' || coalesce(narrative, '') || ' ' || coalesce(feedback, '')));

-- 10. user_memories_identities
DROP INDEX IF EXISTS user_memories_identities_fts_idx;
CREATE INDEX user_memories_identities_fts_idx ON user_memories_identities USING GIN (to_tsvector('english', coalesce(description, '') || ' ' || coalesce(role, '')));

-- 11. user_memories_experiences
DROP INDEX IF EXISTS user_memories_experiences_fts_idx;
CREATE INDEX user_memories_experiences_fts_idx ON user_memories_experiences USING GIN (to_tsvector('english', coalesce(situation, '') || ' ' || coalesce(reasoning, '') || ' ' || coalesce(possible_outcome, '') || ' ' || coalesce(action, '') || ' ' || coalesce(key_learning, '')));

-- 12. user_memory_persona_documents
DROP INDEX IF EXISTS user_memory_persona_documents_fts_idx;
CREATE INDEX user_memory_persona_documents_fts_idx ON user_memory_persona_documents USING GIN (to_tsvector('english', coalesce(tagline, '') || ' ' || coalesce(persona, '')));

-- 13. documents
DROP INDEX IF EXISTS documents_fts_idx;
CREATE INDEX documents_fts_idx ON documents USING GIN (to_tsvector('english', coalesce(title, '') || ' ' || coalesce(description, '') || ' ' || coalesce(content, '')));

-- 14. messages
DROP INDEX IF EXISTS messages_fts_idx;
CREATE INDEX messages_fts_idx ON messages USING GIN (to_tsvector('english', coalesce(content, '') || ' ' || coalesce(summary, '')));
